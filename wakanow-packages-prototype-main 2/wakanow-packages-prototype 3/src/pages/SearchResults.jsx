...
import { cardPrice, pricePackage } from '../lib/pricing.js';
...
export default function SearchResults() {
  ...
  const { search, nights, dateLabel, travellerSummary, setTier, party, partyLabel, isSharingBasis } =
    useTrip();
  ...
  const shareOnWhatsApp = (tier, price) => {
    const text = `${tier.name} package — ${naira(price)} per adult sharing — Wakanow`;
        ...
        <div className="wrap">
          ...
          <div className="tiers">
            {TIERS.map((tier) => {
              ...
              // The headline is the comparable one — two adults in one room —
              // so a party that is anything else is told its own total here
              // rather than discovering it at checkout.
              const forParty = pricePackage(tier, { nights, party });
                ...
                >
                  ...
                  <div className="tiertop">
                    ...
                    <div className="tierprice">
                      <span className="amt">{naira(now)}</span> <span className="pp">/adult sharing</span>
                    </div>
                    <div className="tierwas">{naira(was)} booked separately</div>
                    {!isSharingBasis && (
                      <div className="tierwas">
                        {naira(forParty.partyBundled)} for {partyLabel}
                        {forParty.rooms > 1 ? ` · ${forParty.rooms} rooms` : ''}
                      </div>
                    )}
